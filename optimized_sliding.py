...
def compute_metrics(window: WindowSketch, reference: ReferenceSketch) -> dict[str, float | int]:
    ...
    window_hashes = window.hash_to_weight
    reference_hashes = reference.hash_to_weight
    smaller = window_hashes
    larger = reference_hashes
    smaller_is_window = True
    if len(smaller) > len(larger):
        smaller, larger = larger, smaller
        smaller_is_window = False

    intersection_count = 0
    shared_window_weight = 0.0
    shared_reference_weight = 0.0
    shared_min_weight = 0.0
    dot_product = 0.0
    for hash_value, smaller_weight in smaller.items():
        larger_weight = larger.get(hash_value)
        if larger_weight is None:
            continue

        intersection_count += 1
        if smaller_is_window:
            window_weight = smaller_weight
            reference_weight = larger_weight
        else:
            window_weight = larger_weight
            reference_weight = smaller_weight
        shared_window_weight += window_weight
        shared_reference_weight += reference_weight
        shared_min_weight += min(window_weight, reference_weight)
        dot_product += window_weight * reference_weight
...
def open_output_for_run(
    ...
    if args.output.exists():
        ...
        handle = args.output.open(
            "a",
            newline="",
            encoding="utf-8",
            buffering=DEFAULT_OUTPUT_BUFFER_SIZE,
        )
